...
class FileSystem {
  public:
    ...
    FileSystem() {
		...
		{
			openfile[i] = NULL;
			//openfile[i]->filename=NULL;
		}
		this->Create("stdin"); 
		this->Create("stdout"); 

		//tạo chỗ cho hai file 0 và 1 từ console
		OpenFile* temp = this->Open("stdin"); // index = 1			
		openfile[0] = temp;		// index = 1
		openfile[0]->type = 1;
		//openfile[0]->filename= new char [strlen("stdin")+1];
		filename[0]="stdin";
		temp = this->Open("stdout");
		openfile[1] = temp;	// index = 2
		openfile[1]->type = 0;
		// openfile[1]->filename= new char [strlen("stdin")+1];
		filename[1]="stdout";
	}
	int isFull()
	{
	...
	OpenFile* socketTCP() {	
		...
		return new socketH(fileDescriptor); 
	}
